...
class Canvas {
    ...
    async init() {
        ...
        Object.defineProperty(this, 'pixelData', {
            value: ndarray(new Float64Array(canvasWidth * canvasHeight), [canvasWidth, canvasHeight]),
            writable: false,
            enumerable: true,
            configurable: false,
        });
    ...
    async loadPixelData(pixel) {
        var [x, y, col] = pixel;
        this.pixelData.set(x, y, col);
    }

    async getDimensions() {
        var res = await fetch("https://pixelplace.io/api/get-painting.php?id=" + this.boardId + "&connected=1", {
        "headers": {
        ...
        res = await res.json();
        return { width: res.painting.width, height: res.painting.height }
    }
}
