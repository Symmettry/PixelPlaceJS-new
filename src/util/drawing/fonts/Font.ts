...
type BuiltInFont = "SMALL_FONT" | "MEDIUM_FONT";
...
export function cacheText<FD extends FontData, T extends TextData<FD>>(textData: T): CachedTextData<T['font']> {
    ...
    const spaces: UncoloredSpotData = box(textData.spaceLength, font.height);
    const separator: UncoloredSpotData = box(textData.separatorLength, font.height);

    let lastData: null | [text: string, x: number, y: number] = null, save: null | TextPositions = null;
    ...
    reg.getCharData = (textData: TextData<FD>, x: number, y: number): TextPositions => {
        ...
        if(lastData) {
            let [lt, lx, ly] = lastData;
            if(lt == text && lx == x && ly == y) {
                if(save) return save;
                shouldStore = true;
            } else {
                save = null;
        ...
        for(let i=0;i<chars.length;i++) {
            ...
            const spotData: SpotData = c == " " ? spaces : reg.characterSpotData[c];
            data.push(...spotData.map(applyChar));
